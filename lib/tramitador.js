...
class Tramitador{
    ...
    tramitarPreaccion(proceso, args){

        this.debug(`Procesando preaccion ${proceso}`)
        ...
        return this.procesador.ejecutar(tarea).catch(({resultados}) => {

            //en caso de error, devolvemos una respuesta

            args.res.setHeader('Content-Type', 'application/json');

            let e = tarea.resultados.__error__;

            args.res.statusCode = e.codigo;    

            args.res.end(JSON.stringify({"error": this.__acortarError(e.mensaje)}));
    ...
    _tramitar(tarea, p){
        ...
        return this.procesador.ejecutar(tarea)
